...
task get_daily_report: :environment do
  puts "Starting daily reports"
  puts "Getting daily electronic titles report"
  ...
  headers = {
    params: {
      CGI.escape('path') => '/shared/Galileo Network/Reports/New Titles Electronic Daily',
      CGI.escape('limit') => '1000',
      CGI.escape('apikey') => Rails.application.secrets.api_key
    }
  }

  billing_report_xml = RestClient::Request.execute(
    method: 'GET',
    url: url,
    headers: headers
  )
  ...
  while is_finished != 'true'
    headers_redo = {
      params: {
        CGI.escape('token') => resume_token,
        CGI.escape('limit') => '1000',
        CGI.escape('apikey') => Rails.application.secrets.api_key
      }
    }
    billing_report_xml_redo = RestClient::Request.execute(
      method: 'GET',
      url: url,
      headers: headers_redo
    )
  ...
  xml_rows.each do |row|
    titles << Title.create()
    #sql ="INSERT INTO titles (first_name, last_name, middle_name, institution,
    # primary_identifier, preferred_email, created_at, updated_at) VALUES
    # ('#{block[0]}', '#{block[1]}', '#{block[2]}', '#{block[3]}', '#{block[4]}', '#{block[5]}',  '#{Date.today}', '#{Date.today}');"
  end
